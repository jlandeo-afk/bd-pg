-- Function: questions.fn_indexar_question(integer, integer, json, integer, integer, character varying, numeric, character varying, integer, character varying, integer)

--

CREATE FUNCTION questions.fn_indexar_question(p_id integer, p_topic_id integer, p_subtopics json, p_level_id integer, p_alternative_id integer, p_type character varying, p_ter numeric, p_editor_content character varying, p_updated_by integer, p_status character varying, p_employee_question_id integer DEFAULT NULL::integer) RETURNS TABLE(new_question_id integer, new_status character varying, v_url_pdf character varying)
    LANGUAGE plpgsql
    AS $$
        DECLARE
            v_subtopic_id INT;
        ...
        BEGIN
            new_status := (SELECT status FROM questions.question WHERE id = p_id);
            ...
            -- Consultar los IDs existentes en questions.question_subtopic
            v_existing_subtopic_ids := ARRAY(
                SELECT subtopic_id
                FROM questions.question_subtopic
            ...
            IF p_editor_content IS NOT NULL AND p_employee_question_id IS NOT NULL THEN
                UPDATE questions.employee_question e_q
...
ALTER FUNCTION questions.fn_indexar_question(p_id integer, p_topic_id integer, p_subtopics json, p_level_id integer, p_alternative_id integer, p_type character varying, p_ter numeric, p_editor_content character varying, p_updated_by integer, p_status character varying, p_employee_question_id integer) OWNER TO postgres;
