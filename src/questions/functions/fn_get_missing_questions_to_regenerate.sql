-- Function: questions.fn_get_missing_questions_to_regenerate(date, text[])

--

CREATE FUNCTION questions.fn_get_missing_questions_to_regenerate(p_since_date date, p_excluded_statuses text[]) RETURNS TABLE(question_id bigint, question_status character varying)
    LANGUAGE plpgsql
    AS $$
        BEGIN
            RETURN QUERY
...
ALTER FUNCTION questions.fn_get_missing_questions_to_regenerate(p_since_date date, p_excluded_statuses text[]) OWNER TO postgres;
