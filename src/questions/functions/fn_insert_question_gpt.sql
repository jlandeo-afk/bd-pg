-- Function: questions.fn_insert_question_gpt(text, text, smallint, jsonb, jsonb, smallint, smallint, smallint, bigint, bigint)

--

CREATE FUNCTION questions.fn_insert_question_gpt(p_question_text text, p_question_short text, p_course_id smallint, p_alternatives jsonb, p_solutions jsonb, p_topic_id smallint, p_subtopic_id smallint, p_level_id smallint, p_config_alternative_id bigint, p_created_by bigint) RETURNS TABLE(question_id bigint, question_code character varying)
    LANGUAGE plpgsql
    AS $$
			DECLARE
            ...
            BEGIN
                ...
                SELECT code INTO v_code_course FROM academic.course WHERE id = p_course_id LIMIT 1;

                -- Obtener numero de pregunta
                SELECT LPAD(CAST(COALESCE(CAST(MAX(number) AS INTEGER) + 1, 1) AS TEXT), 7, '0') INTO v_number FROM questions.question WHERE course_id = p_course_id AND fl_status = true LIMIT 1;
                ...
                INSERT INTO questions.question_subtopic (question_id, subtopic_id, created_by, created_at)
                ...
                FOR v_alternative IN SELECT jsonb_array_elements(p_alternatives) LOOP
                    INSERT INTO questions.alternative (description, question_id, created_by, created_at) VALUES ((v_alternative->>'text')::TEXT, v_question_id, p_created_by, now())
                    ...
                    IF ((v_alternative->>'answer')::BOOLEAN IS TRUE) THEN
                        ...
                        UPDATE questions.question SET answer_id = v_answer_id WHERE id = v_question_id;
                    END IF;
                END LOOP;
                ...
                INSERT INTO questions.employee_question (teacher_id, question_id, alternative_id, type, week, year, solved, created_by, created_at)
                ...
                INSERT INTO questions.employee_didi_question (employee_id, question_id, week, year, diagrammed, created_by, created_at)
                ...
                FOR v_solution IN SELECT jsonb_array_elements(p_solutions) LOOP
                    INSERT INTO questions.employee_didi_question_field (employee_didi_question_id, setting_diagrammed_course_id, value, created_by, created_at)
...
ALTER FUNCTION questions.fn_insert_question_gpt(p_question_text text, p_question_short text, p_course_id smallint, p_alternatives jsonb, p_solutions jsonb, p_topic_id smallint, p_subtopic_id smallint, p_level_id smallint, p_config_alternative_id bigint, p_created_by bigint) OWNER TO postgres;
