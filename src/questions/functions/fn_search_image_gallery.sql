-- Function: questions.fn_search_image_gallery(integer, integer, character varying, integer, integer)

--

CREATE FUNCTION questions.fn_search_image_gallery(p_perpage integer, p_npage integer, p_name_text character varying, f_course_id integer, f_topic_id integer) RETURNS TABLE(id bigint, name character varying, image character varying, width integer, height integer, path_short_version character varying, path_full_version character varying, created_by bigint, created_at timestamp without time zone, topics jsonb, total_count bigint)
    LANGUAGE plpgsql
    AS $$
            DECLARE
                offset_val INTEGER;
                ...
                WITH filtered_image_gallery AS (
                    SELECT
                        ...
                        (
                            ...
                            FROM questions.image_galery_topic igt
                            LEFT JOIN academic.topic t ON igt.topic_id = t.id
                    ...
                    FROM questions.image_gallery ig
...
ALTER FUNCTION questions.fn_search_image_gallery(p_perpage integer, p_npage integer, p_name_text character varying, f_course_id integer, f_topic_id integer) OWNER TO postgres;
