-- Function: questions.get_didis_for_assignment(integer, integer, integer)

--

CREATE FUNCTION questions.get_didis_for_assignment(p_week integer, p_year integer, p_course_id integer) RETURNS TABLE(id bigint, assign_counts jsonb, name text)
    LANGUAGE plpgsql
    AS $$
    BEGIN
        RETURN QUERY
...
ALTER FUNCTION questions.get_didis_for_assignment(p_week integer, p_year integer, p_course_id integer) OWNER TO postgres;
