-- Function: questions.fn_get_missing_text_with_subquestions(integer, integer, integer[])

--

CREATE FUNCTION questions.fn_get_missing_text_with_subquestions(p_material_id integer, p_week integer, p_type_material_ids integer[]) RETURNS TABLE(material character varying, course character varying, category character varying, subcategory character varying, level character varying, topic character varying, subtopic character varying, distribution_id bigint)
    LANGUAGE plpgsql
    AS $$
                    BEGIN
                        RETURN QUERY
...
ALTER FUNCTION questions.fn_get_missing_text_with_subquestions(p_material_id integer, p_week integer, p_type_material_ids integer[]) OWNER TO postgres;
