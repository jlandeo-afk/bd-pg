-- Function: questions.fn_amount_parent_question_level(bigint, smallint, smallint, jsonb)

--

CREATE FUNCTION questions.fn_amount_parent_question_level(p_material_id bigint, p_type_material_id smallint, p_week smallint, p_courses jsonb) RETURNS TABLE(number_of_passages integer, level_id bigint, level_name character varying, description character varying, course_id bigint)
    LANGUAGE plpgsql
    AS $$
	DECLARE
            v_cycle_id BIGINT;
...
ALTER FUNCTION questions.fn_amount_parent_question_level(p_material_id bigint, p_type_material_id smallint, p_week smallint, p_courses jsonb) OWNER TO postgres;
