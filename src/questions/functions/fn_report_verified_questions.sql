-- Function: questions.fn_report_verified_questions(integer, integer, integer, integer[])

--

CREATE FUNCTION questions.fn_report_verified_questions(p_course_id integer, p_topic_id integer, p_subtopic_id integer, p_level_id integer[]) RETURNS TABLE(correlativo bigint, course_name character varying, topic_name character varying, subtopic_name character varying, level character varying, total_question bigint)
    LANGUAGE plpgsql
    AS $$
        BEGIN
            RETURN QUERY 
...
ALTER FUNCTION questions.fn_report_verified_questions(p_course_id integer, p_topic_id integer, p_subtopic_id integer, p_level_id integer[]) OWNER TO postgres;
