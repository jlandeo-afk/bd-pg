-- Function: questions.fn_insert_question_with_alternatives(character varying, character varying, character varying, character varying, smallint, text, text, integer, jsonb, jsonb, jsonb, bigint, character varying, text, character varying, character varying, integer, character varying, integer, integer, character varying, character varying, character varying, boolean, bigint, integer, integer, integer)

--

CREATE FUNCTION questions.fn_insert_question_with_alternatives(p_code character varying, p_code_course character varying, p_num_problem character varying, p_num character varying, p_topic_id smallint, p_question_text text, p_question_short text, p_parent_id integer, p_question_images jsonb, p_alternatives jsonb, p_alternative_images jsonb, p_config_alternative_id bigint, p_observation character varying, p_similary text, p_status character varying, p_region character varying, p_university integer, p_year character varying, p_option integer, p_modality integer, p_version character varying, p_area character varying, p_authorship character varying, p_diagrammed boolean, p_created_by bigint, p_cycle_id integer, p_week_id integer, p_type_material_id integer) RETURNS integer
    LANGUAGE plpgsql
    AS $$
            DECLARE
                question_id bigint;
            ...
            BEGIN
                ...
                INSERT INTO questions.question (code, description, short_description, number_question, number, course_id, topic_id, parent_id, status, region, university_id, year, option_id, modality_id, config_alternative_id, version, area, authorship, fl_diagrammed, created_by, created_at)
                ...
                FOR image_question IN SELECT * FROM jsonb_array_elements(p_question_images) LOOP
                    INSERT INTO questions.question_image (code, extension, image, question_id, created_by, created_at)
                ...
                FOR alternative IN SELECT * FROM jsonb_array_elements(p_alternatives) LOOP
                    INSERT INTO questions.alternative (description, question_id, created_by, created_at)
                ...
                IF p_observation IS NOT NULL AND p_observation <> '' THEN
                    INSERT INTO questions.question_observation (description, similitaries, question_id, type, fl_status, created_by, created_at)
                ...
                IF p_cycle_id IS NOT NULL OR p_week_id IS NOT NULL OR p_type_material_id IS NOT NULL THEN
                    INSERT INTO questions.question_temporary (question_id, cycle_id, week_id, type_material_id, created_by, created_at)
...
ALTER FUNCTION questions.fn_insert_question_with_alternatives(p_code character varying, p_code_course character varying, p_num_problem character varying, p_num character varying, p_topic_id smallint, p_question_text text, p_question_short text, p_parent_id integer, p_question_images jsonb, p_alternatives jsonb, p_alternative_images jsonb, p_config_alternative_id bigint, p_observation character varying, p_similary text, p_status character varying, p_region character varying, p_university integer, p_year character varying, p_option integer, p_modality integer, p_version character varying, p_area character varying, p_authorship character varying, p_diagrammed boolean, p_created_by bigint, p_cycle_id integer, p_week_id integer, p_type_material_id integer) OWNER TO postgres;
