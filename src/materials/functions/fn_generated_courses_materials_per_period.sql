-- Function: materials.fn_generated_courses_materials_per_period(integer, character varying, boolean, integer)

--

CREATE FUNCTION materials.fn_generated_courses_materials_per_period(p_material_per_period_ballot_id integer, p_type character varying, p_has_permission_generar_por_cursos_asignados boolean, p_employee_id integer) RETURNS TABLE(id bigint, url_resource character varying, course_id smallint, course character varying, status_name text, resource_name text)
    LANGUAGE plpgsql
    AS $$
        BEGIN
            RETURN QUERY
...
ALTER FUNCTION materials.fn_generated_courses_materials_per_period(p_material_per_period_ballot_id integer, p_type character varying, p_has_permission_generar_por_cursos_asignados boolean, p_employee_id integer) OWNER TO postgres;
