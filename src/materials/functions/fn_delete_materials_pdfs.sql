-- Function: materials.fn_delete_materials_pdfs(bigint, smallint, jsonb, bigint)

--

CREATE FUNCTION materials.fn_delete_materials_pdfs(p_material_id bigint, p_week smallint, p_type_material_ids jsonb, p_deleted_by bigint) RETURNS void
    LANGUAGE plpgsql
    AS $$
        DECLARE
            v_fl_exam BOOLEAN;
        ...
        BEGIN
            ...
            FOR v_type_material_id IN SELECT jsonb_array_elements(p_type_material_ids) LOOP
                ...
                SELECT tm.fl_exam INTO v_fl_exam FROM materials.type_material tm WHERE tm.id = v_type_material_id LIMIT 1;

                -- Obtener el id del material de la semana y tipo material mandado y eliminar las rutas de los materiales
                UPDATE academic.detail_week_type_mat detm
                ...
                IF v_week_type_material_id IS NOT NULL THEN
                    IF v_fl_exam IS TRUE THEN
                        FOR v_material_exam_area_week_id IN
                            SELECT meaw.id
                            FROM materials.material_exam_area_week meaw
                        ...
                        LOOP
                            ...
                            UPDATE materials.material_exam_area_week meaw1
...
ALTER FUNCTION materials.fn_delete_materials_pdfs(p_material_id bigint, p_week smallint, p_type_material_ids jsonb, p_deleted_by bigint) OWNER TO postgres;
