    ...
    AS $$
        ...
        BEGIN
            ...
            WITH tlb_question_materials AS (
                SELECT
                    ...
                    (
                        ...
                        LEFT JOIN LATERAL (
                            ...
                            FROM questions.alternative a WHERE a.question_id = mbs.question_id AND a.fl_status = true
                        ) vqa ON TRUE
                        LEFT JOIN LATERAL (
                            ...
                            FROM questions.employee_didi_question ed
                            JOIN questions.employee_didi_question_field edf ON ed.id = edf.employee_didi_question_id AND edf.fl_status = true
                            JOIN academic.setting_diagrammed_courses sd ON edf.setting_diagrammed_course_id = sd.id AND sd.fl_active = true AND sd.fl_status = true
                            JOIN questions.field_diagrammed fd ON sd.field_diagram_id = fd.id
                ...
                LEFT JOIN LATERAL (
                    SELECT jsonb_agg(jsonb_build_object('id', a.id, 'description', a.description) ORDER BY a.id) AS alternatives
                    FROM questions.alternative a WHERE a.question_id = q.id AND a.fl_status = true
                ) alt ON TRUE
                LEFT JOIN LATERAL (
                    SELECT jsonb_agg(jsonb_build_object('id', edf.id, 'value', edf.value, 'position', sd."position", 'field_diagram', fd.name) ORDER BY sd.position) AS diagrammed
                    FROM questions.employee_didi_question ed
                ...
                LEFT JOIN LATERAL (
                    ...
                    FROM questions.employee_didi_question ed
                    JOIN questions.employee_didi_question_field_image edfi ON ed.id = edfi.employee_didi_question_id AND edfi.fl_status = true
                ...
                LEFT JOIN LATERAL (
                    ...
                    FROM questions.question_image qi WHERE qi.question_id = q.id AND qi.fl_status = true
                ) img ON TRUE
                LEFT JOIN LATERAL (
                    SELECT jsonb_agg(jsonb_build_object('id', qm.id, 'code', qm.code, 'path', qm.path, 'properties', qm.properties)) AS question_maths
                    FROM questions.question_maths qm WHERE qm.question_id = q.id AND qm.fl_status = true
                ) math ON TRUE
                LEFT JOIN LATERAL (
                    ...
                    JOIN questions.alternative_maths am ON a.id = am.alternative_id AND am.fl_status = true
                ...
                LEFT JOIN LATERAL (
                    ...
                    FROM questions.employee_didi_question ed
                    JOIN questions.employee_didi_question_field edf ON ed.id = edf.employee_didi_question_id AND edf.fl_status = true
                    JOIN questions.didi_maths dm ON edf.id = dm.didi_question_id AND dm.fl_status = true
                    WHERE ed.question_id = q.id AND ed.fl_status = true
                ) vdm ON TRUE
                LEFT JOIN LATERAL (
                    SELECT jsonb_agg(jsonb_build_object('id', pi.id, 'code', pi.code, 'image', pi.image)) AS parent_img
                    FROM questions.parent_image pi WHERE pi.parent_id = pq.id AND pi.fl_status = true
                ) vpdm ON TRUE
...
ALTER FUNCTION questions.fn_question_material_ballot_pdf(p_week_type_materials jsonb, p_course_id smallint) OWNER TO postgres;
