...
BEGIN
    ...
    WITH ballot_with_questions AS (
        ...
        LEFT JOIN questions.question q ON q.id = mbq.question_id
        INNER JOIN academic.course c ON mbq.course_id = c.id
        LEFT JOIN academic.topic t ON t.id = mbq.topic_id
        LEFT JOIN academic.subtopic st ON st.id = mbq.subtopic_id
        LEFT JOIN academic.level l ON mbq.level_id = l.id
        WHERE mbq.fl_status IS TRUE
        AND mbq.week_type_material_id = p_week_type_material_id
        AND mbq.question_id = p_question_id
    ),
    slot_to_update AS (
        UPDATE materials.material_ballot_question mbqu
        SET
            usage_level_id = NULL,
            usage_status = TRUE,
            question_id = NULL,
...
ALTER FUNCTION materials.fn_remove_question_from_material_ballot(p_week_type_material_id integer, p_deleted_by integer, p_question_id integer) OWNER TO postgres;
