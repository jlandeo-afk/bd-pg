-- Function: questions.fn_delete_question_materials_by_areas_get_questions(bigint, smallint, jsonb, bigint, jsonb, boolean)

--

CREATE FUNCTION questions.fn_delete_question_materials_by_areas_get_questions(p_material_id bigint, p_week smallint, p_type_material_ids jsonb, p_deleted_by bigint, p_areas jsonb, p_fl_validate_code boolean) RETURNS TABLE(id bigint, code character varying, course_id smallint, topic_id smallint, level_id smallint, level smallint, type character varying, status character varying, subtopic_id integer, frequent_subtopic boolean, cycle_id integer, week smallint, type_material_id smallint, history jsonb)
    LANGUAGE plpgsql
    AS $$
        DECLARE
            v_cycle_id BIGINT;
        ...
        BEGIN
            ...
            SELECT
                ...
                qnew.history
            FROM fn_question_verified_material(p_material_id, p_fl_validate_code) qnew;
        END;
        $$;


ALTER FUNCTION questions.fn_delete_question_materials_by_areas_get_questions(p_material_id bigint, p_week smallint, p_type_material_ids jsonb, p_deleted_by bigint, p_areas jsonb, p_fl_validate_code boolean) OWNER TO postgres;
