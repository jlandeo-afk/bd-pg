-- Function: materials.fn_get_config_level_register_ballot_period(bigint, smallint, smallint, smallint)

--

CREATE FUNCTION materials.fn_get_config_level_register_ballot_period(p_material_id bigint, p_type_material_id smallint, p_start_week smallint, p_end_week smallint) RETURNS TABLE(id bigint, type_material_id bigint, amount_type_mat integer, level_id bigint, level_description character varying, level_name character varying, type_question character varying, week smallint, headquarter_id bigint, course_id bigint, code_course character varying, course character varying)
    LANGUAGE plpgsql
    AS $$
                DECLARE
                    v_ids_children INT[];
                BEGIN
                    ...
                    WITH tbl_material AS ( -- Se obtiene el ciclo y universidad del material
                        ...
                        JOIN materials.type_material_detail_courses tmdc ON tmc.id = tmdc.type_material_course_id
...
ALTER FUNCTION materials.fn_get_config_level_register_ballot_period(p_material_id bigint, p_type_material_id smallint, p_start_week smallint, p_end_week smallint) OWNER TO postgres;
