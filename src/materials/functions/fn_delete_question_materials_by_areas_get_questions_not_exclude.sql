-- Function: questions.fn_delete_question_materials_by_areas_get_questions_not_exclude(bigint, smallint, jsonb, bigint, jsonb, boolean)

--

CREATE FUNCTION questions.fn_delete_question_materials_by_areas_get_questions_not_exclude(p_material_id bigint, p_week smallint, p_type_material_ids jsonb, p_deleted_by bigint, p_areas jsonb, p_fl_validate_code boolean) RETURNS TABLE(id bigint, code character varying, course_id smallint, topic_id smallint, level_id smallint, level smallint, type character varying, status character varying, subtopic_id integer, frequent_subtopic boolean, cycle_id integer, week smallint, type_material_id smallint, history jsonb)
    LANGUAGE plpgsql
    AS $$
                DECLARE
                    v_cycle_id BIGINT;
                        ...
                        IF v_week_type_material_id IS NOT NULL THEN
                            ...
                            IF v_type_material.fl_exam IS TRUE THEN
                                ...
                                WITH updated_meaw AS (
                                    ...
                                    AND meaw.area_id = ANY (SELECT jsonb_array_elements_text(p_areas)::bigint)
                            ...
                            ELSE
                                ...
                                updated_mbs AS (
                                    ...
                                    SET fl_status = false,
                                        deleted_at = now()
                                    WHERE mbs.material_ballot_question_id IN (SELECT id FROM updated_mbq)
                                )
...
ALTER FUNCTION questions.fn_delete_question_materials_by_areas_get_questions_not_exclude(p_material_id bigint, p_week smallint, p_type_material_ids jsonb, p_deleted_by bigint, p_areas jsonb, p_fl_validate_code boolean) OWNER TO postgres;
