-- Function: organization.update_data_employee(uuid, bigint, character varying, character varying, character varying, integer, character varying, character varying, character varying, jsonb, integer, jsonb)

--

CREATE FUNCTION organization.update_data_employee(p_uuid uuid, p_charge_id bigint, p_first_name character varying, p_first_surname character varying, p_second_surname character varying, p_type_document_id integer, p_document character varying, p_email character varying, p_phone character varying, p_course_ids jsonb, p_updated_by integer, p_data_docente jsonb) RETURNS TABLE(v_id_employee bigint, v_question_ids integer[])
    LANGUAGE plpgsql
    AS $$
            DECLARE
