...
CREATE FUNCTION auth.fn_update_user_by_uuid(p_user_uuid uuid, p_email character varying, p_role_id bigint, p_company_id integer, p_update_by bigint) RETURNS TABLE(status boolean, data uuid)
    LANGUAGE plpgsql
    AS $$
    BEGIN
        RETURN QUERY
