...
CREATE OR REPLACE FUNCTION auth.fn_update_rol_with_permission(p_id bigint, p_name character varying, p_slug character varying, p_description character varying, p_permissions jsonb, p_updated_by bigint, p_company_id bigint) RETURNS TABLE(id bigint)
    LANGUAGE plpgsql
    AS $$
    BEGIN
        RETURN QUERY
