-- Function: auth.fn_create_user(uuid, character varying, character varying, character varying, integer, character varying, integer, integer, character varying, integer)

CREATE OR REPLACE FUNCTION auth.fn_create_user(p_uuid uuid, p_user_name character varying, p_email character varying, p_password character varying, p_status_session integer, p_image character varying, p_company integer, p_role_id integer, p_employee_uuid character varying, p_created_by integer) RETURNS TABLE(status boolean, data integer)
    LANGUAGE plpgsql
    AS $$
            DECLARE
...
ALTER FUNCTION auth.fn_create_user(p_uuid uuid, p_user_name character varying, p_email character varying, p_password character varying, p_status_session integer, p_image character varying, p_company integer, p_role_id integer, p_employee_uuid character varying, p_created_by integer) OWNER TO postgres;
