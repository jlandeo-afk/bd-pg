-- Function: exams.fn_search_exam_area(integer, integer, character varying, integer, boolean, integer, integer)

--

CREATE FUNCTION exams.fn_search_exam_area(p_perpage integer, p_npage integer, p_name_text character varying, f_cycle_id integer, f_cycle_active boolean, f_type_material_id integer, p_company_id integer) RETURNS TABLE(id smallint, cycle_id bigint, cycle character varying, type_material_id smallint, type_material character varying, created_at timestamp without time zone, created_by text, is_exam_material_configured boolean, total_count bigint)
    LANGUAGE plpgsql
    AS $$
    DECLARE
        offset_val INTEGER;
...
ALTER FUNCTION exams.fn_search_exam_area(p_perpage integer, p_npage integer, p_name_text character varying, f_cycle_id integer, f_cycle_active boolean, f_type_material_id integer, p_company_id integer) OWNER TO postgres;
