-- Function: exams.fn_exam_key_generation(integer, integer, integer, integer, jsonb, integer, integer, integer, integer, integer)

--

CREATE FUNCTION exams.fn_exam_key_generation(p_perpage integer, p_npage integer, p_cycle_id integer, p_exam_area_id integer, p_week jsonb, p_cycle_type_id integer, p_cycle_active integer, p_university_id integer, p_exam_type_template_id integer, p_company_id integer) RETURNS TABLE(id bigint, integer_date integer, cycle character varying, cycle_name character varying, formated_date character varying, week smallint, area character varying, exam_type character varying, cycle_type character varying, code character varying, total_count bigint)
    LANGUAGE plpgsql
    AS $$
                DECLARE
                    offset_val INTEGER;
...
ALTER FUNCTION exams.fn_exam_key_generation(p_perpage integer, p_npage integer, p_cycle_id integer, p_exam_area_id integer, p_week jsonb, p_cycle_type_id integer, p_cycle_active integer, p_university_id integer, p_exam_type_template_id integer, p_company_id integer) OWNER TO postgres;
