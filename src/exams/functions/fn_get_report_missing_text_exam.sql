...
CREATE FUNCTION exams.fn_get_report_missing_text_exam(p_material_id integer, p_type_material_id integer, p_week integer) RETURNS TABLE(course_id smallint, course character varying, material_exam_id integer, area character varying, type_text_id smallint, type_text_subcategory_id smallint, category character varying, subcategory character varying, topics jsonb)
    ...
    AS $$
                            ...
                            all_areas AS (
                                SELECT meaw.id material_exam_id, ea.description area, meaw.area_id FROM academic.detail_week_type_mat AS dwtm
...
ALTER FUNCTION exams.fn_get_report_missing_text_exam(p_material_id integer, p_type_material_id integer, p_week integer) OWNER TO postgres;
