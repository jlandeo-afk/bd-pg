            ...
            BEGIN
                ...
                WITH question_per_material_exam_area_week AS (
                    SELECT
                        ...
                        m.university_id
                    FROM materials.material_exam_question AS meq
                    LEFT JOIN academic.topic AS t ON meq.topic_id = t.id
                    LEFT JOIN academic.subtopic AS st ON st.id = meq.subtopic_id
                    JOIN questions.question AS q ON q.id = meq.question_id AND meq.fl_status IS TRUE
                    LEFT JOIN materials.material_exam_area_week AS meaw ON meaw.id = meq.material_exam_area_week_id
                    LEFT JOIN exams.exam_area AS ea ON ea.id = meaw.area_id
                    LEFT JOIN academic.detail_week_type_mat AS dwtm ON dwtm.id = meaw.week_type_material_id
                    LEFT JOIN materials.material AS m ON m.id = dwtm.material_id
                    LEFT JOIN academic.cycle AS c ON c.id = m.cycle_id
                    LEFT JOIN academic.cycle_types ct ON ct.id = c.cycle_type_id
                    LEFT JOIN materials.type_material AS tm ON tm.id = dwtm.type_material_id
                    LEFT JOIN materials.type_material_template AS tmt ON tmt.id = tm.type_material_template_id
                    LEFT JOIN materials.template_type_material_courses_order AS co ON co.template_type_material_id = tmt.id AND meq.course_id = co.course_id
                        AND co.university_id IS NOT DISTINCT FROM v_university_id
                    LEFT JOIN academic.level AS l ON l.id = meq.level_id
                    WHERE 1 = 1
                    ...
                    SELECT
                        ...
                        m.university_id
                    FROM materials.material_exam_parent_question AS mepq
                    JOIN questions.parent_question AS pq ON pq.id = mepq.parent_question_id
                    LEFT JOIN materials.material_exam_area_week AS meaw ON meaw.id = mepq.material_exam_area_week_id
                    LEFT JOIN exams.exam_area AS ea ON ea.id = meaw.area_id
                    LEFT JOIN academic.detail_week_type_mat AS dwtm ON dwtm.id = meaw.week_type_material_id
                    LEFT JOIN materials.material AS m ON m.id = dwtm.material_id
                    LEFT JOIN academic.cycle AS c ON c.id = m.cycle_id
                    ...
                    LEFT JOIN materials.type_material AS tm ON tm.id = dwtm.type_material_id
                    LEFT JOIN materials.type_material_template AS tmt ON tmt.id = tm.type_material_template_id
                    LEFT JOIN materials.template_type_material_courses_order AS co ON co.template_type_material_id = tmt.id AND mepq.course_id = co.course_id
                        AND co.university_id IS NOT DISTINCT FROM v_university_id
                    WHERE 1 = 1
                    ...
                    LEFT JOIN questions.alternative AS al ON al.question_id = qn.question_id AND al.fl_status IS TRUE
                    ...
                    JOIN questions.question AS q ON q.id = (sq.value->>'question_id')::BIGINT
                    LEFT JOIN questions.alternative AS al ON al.question_id = q.id AND al.fl_status IS TRUE
...
ALTER FUNCTION exams.fn_report_exam_key_generation_v2(p_ids jsonb) OWNER TO postgres;
