-- Function: academic.fn_paginate_cycles(integer, integer, character varying, boolean, integer)

--

CREATE FUNCTION academic.fn_paginate_cycles(p_perpage integer, p_npage integer, p_name_text character varying, p_active boolean, p_company_id integer) RETURNS TABLE(id bigint, code character varying, description character varying, month smallint, year smallint, weeks smallint, start_date date, end_date date, active boolean, cycle_type_id bigint, cycle_type character varying, created_by text, created_at timestamp without time zone, total_count bigint)
...
ALTER FUNCTION academic.fn_paginate_cycles(p_perpage integer, p_npage integer, p_name_text character varying, p_active boolean, p_company_id integer) OWNER TO postgres;
