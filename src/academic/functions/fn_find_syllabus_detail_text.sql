-- Function: academic.fn_find_syllabus_detail_text(integer, integer, integer, integer)

--

CREATE FUNCTION academic.fn_find_syllabus_detail_text(p_syllabus_id integer, p_category_id integer, p_week integer, p_company_id integer) RETURNS TABLE(material_name character varying, subcategories jsonb)
    LANGUAGE plpgsql
    AS $$
            BEGIN
                RETURN QUERY
...
ALTER FUNCTION academic.fn_find_syllabus_detail_text(p_syllabus_id integer, p_category_id integer, p_week integer, p_company_id integer) OWNER TO postgres;
