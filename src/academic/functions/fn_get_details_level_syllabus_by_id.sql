-- Function: academic.fn_get_details_level_syllabus_by_id(bigint)

--

CREATE FUNCTION academic.fn_get_details_level_syllabus_by_id(p_level_cycle_id bigint) RETURNS TABLE(cycle_id bigint, code_cycle character varying, name_cycle character varying, university_id smallint, name_university character varying, headquarter_id smallint, name_headquarter character varying, course_id smallint, code_course character varying, name_course character varying, total_weeks smallint, cycle_template_id bigint, weeks jsonb)
    LANGUAGE plpgsql
    AS $$
                            BEGIN
                            RETURN QUERY
                            WITH base_level AS (
                                    ...
                                    WHERE ls.id = p_level_cycle_id AND ls.fl_status = TRUE AND ls.deleted_at IS NULL
                                ),
                                expected_raw AS (
                                    SELECT 
                                        tmdt.week,
...
ALTER FUNCTION academic.fn_get_details_level_syllabus_by_id(p_level_cycle_id bigint) OWNER TO postgres;
