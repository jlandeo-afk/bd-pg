-- Function: academic.fn_week_subtopics(bigint, integer, bigint)

--

CREATE FUNCTION academic.fn_week_subtopics(p_syllabus_id bigint, p_week integer, p_company_id bigint) RETURNS TABLE(syllabus_id bigint, week smallint, topic_id smallint, topic_name character varying, subtopic_id smallint, subtopic_name character varying, amount smallint, questions_amount jsonb)
    LANGUAGE plpgsql
    AS $$
        BEGIN
            RETURN QUERY
...
ALTER FUNCTION academic.fn_week_subtopics(p_syllabus_id bigint, p_week integer, p_company_id bigint) OWNER TO postgres;
