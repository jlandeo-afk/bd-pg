-- Function: academic.get_detail_level_syllabus_week_parent(integer, integer, integer, integer, jsonb)

--

CREATE FUNCTION academic.get_detail_level_syllabus_week_parent(p_university_id integer, p_cycle_id integer, p_week_id integer, p_course_id integer, p_headquarter_id jsonb) RETURNS TABLE(level_id bigint, name_level character varying, type_material_id smallint, name_type_material character varying, total_text integer)
    LANGUAGE plpgsql
    AS $$
	BEGIN
		RETURN QUERY
...
ALTER FUNCTION academic.get_detail_level_syllabus_week_parent(p_university_id integer, p_cycle_id integer, p_week_id integer, p_course_id integer, p_headquarter_id jsonb) OWNER TO postgres;
