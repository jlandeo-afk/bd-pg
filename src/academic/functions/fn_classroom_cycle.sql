-- Function: academic.fn_classroom_cycle(integer, integer, smallint, smallint, bigint, smallint)

--

CREATE FUNCTION academic.fn_classroom_cycle(p_perpage integer, p_npage integer, f_university_id smallint, f_headquarters_id smallint, f_cycle_id bigint, f_classroom_id smallint) RETURNS TABLE(cycle_id bigint, code_cycle character varying, description_cycle character varying, university_id smallint, university character varying, headquarters_id smallint, code_headquarters character varying, headquarters character varying, slug_headquarters character varying, classroom_id smallint, code_classroom character varying, classroom character varying, total bigint)
    LANGUAGE plpgsql
    AS $$
        DECLARE
            offset_val INTEGER;
        BEGIN
            ...
            WITH tbl_university AS (
                ...
                FROM academic.origin_university u
                INNER JOIN academic.university_headquarters uh ON u.id = uh.university_id AND uh.fl_status = true
...
ALTER FUNCTION academic.fn_classroom_cycle(p_perpage integer, p_npage integer, f_university_id smallint, f_headquarters_id smallint, f_cycle_id bigint, f_classroom_id smallint) OWNER TO postgres;
