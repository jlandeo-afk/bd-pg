-- Function: academic.fn_courses_complete_management(boolean, bigint)

--

CREATE FUNCTION academic.fn_courses_complete_management(p_fl_pseudo_course boolean, p_employee_id bigint) RETURNS TABLE(course_id smallint, code character varying, name character varying, area_id bigint, count_questions bigint, fl_pseudo_course boolean, children jsonb, type_text_template_id smallint)
    LANGUAGE plpgsql
    AS $$
                BEGIN
                    RETURN QUERY
...
ALTER FUNCTION academic.fn_courses_complete_management(p_fl_pseudo_course boolean, p_employee_id bigint) OWNER TO postgres;
