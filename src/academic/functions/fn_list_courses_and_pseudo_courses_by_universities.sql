-- Function: academic.fn_list_courses_and_pseudo_courses_by_universities(jsonb, integer, jsonb)

--

CREATE FUNCTION academic.fn_list_courses_and_pseudo_courses_by_universities(f_university_ids jsonb, p_company_id integer, f_courses_employee jsonb) RETURNS TABLE(id smallint, code character varying, name character varying, exist_some boolean, fl_pseudo boolean, children jsonb)
    LANGUAGE plpgsql
    AS $$
        DECLARE
...
ALTER FUNCTION academic.fn_list_courses_and_pseudo_courses_by_universities(f_university_ids jsonb, p_company_id integer, f_courses_employee jsonb) OWNER TO postgres;
