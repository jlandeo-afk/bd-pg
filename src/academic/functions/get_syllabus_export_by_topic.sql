-- Function: academic.get_syllabus_export_by_topic(bigint, bigint)

--

CREATE FUNCTION academic.get_syllabus_export_by_topic(p_syllabus_id bigint, p_company_id bigint) RETURNS TABLE(syllabus_id bigint, course_name character varying, cycle_description character varying, week integer, topic_id smallint, week_name character varying, alternative_week_name text, topic_name character varying, subtopics jsonb)
    LANGUAGE plpgsql
    AS $$
     DECLARE v_weeks_amount INTEGER DEFAULT 0;
 BEGIN
     SELECT c.weeks INTO v_weeks_amount
     FROM academic.syllabus syll
     JOIN academic.cycle c ON c.id = syll.cycle_id
     ...
     RETURN QUERY
         WITH syllabus_weeks AS (
             ...
             FROM academic.syllabus syll
             INNER JOIN academic.syllabus_topic st ON st.syllabus_id = syll.id AND st.fl_status IS TRUE AND st.is_topic_deleted IS FALSE
             INNER JOIN academic.syllabus_topic_week stw ON stw.syllabus_topic_id = st.id AND stw.fl_status IS TRUE
             INNER JOIN academic.syllabus_detail_subtopic sds ON sds.syllabus_topic_week_id = stw.id AND sds.fl_status IS TRUE AND sds.is_subtopic_deleted IS FALSE
             WHERE syll.id = p_syllabus_id
             AND syll.company_id = p_company_id
             ...
             LEFT JOIN academic.syllabus_subtopic_type_material sst ON sst.syllabus_detail_subtopic_id = sgc.syllabus_detail_subtopic_id
             ...
             JOIN academic.subtopic st ON slc.subtopic_id = st.id
             ...
             JOIN academic.course c ON c.id = swt.course_id
             JOIN academic.topic t ON t.id = swt.topic_id
             JOIN academic.cycle cy ON cy.id = swt.cycle_id
             LEFT JOIN academic.syllabus_week_titles stt ON stt.syllabus_id = swt.id AND swt.week = stt.week AND stt.fl_status IS TRUE
...
ALTER FUNCTION academic.get_syllabus_export_by_topic(p_syllabus_id bigint, p_company_id bigint) OWNER TO postgres;
