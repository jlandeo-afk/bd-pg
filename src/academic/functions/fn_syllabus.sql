-- Function: academic.fn_syllabus(integer, integer, jsonb, jsonb, bigint, boolean, bigint)

--

CREATE FUNCTION academic.fn_syllabus(p_perpage integer, p_npage integer, f_course_ids jsonb, f_university_ids jsonb, f_cycle_id bigint, f_cycle_active boolean, p_company_id bigint) RETURNS TABLE(id bigint, university_id smallint, code_university character varying, slug_university character varying, university character varying, course_id bigint, code_course character varying, course character varying, cycle_id bigint, code_cycle character varying, cycle character varying, start_date date, end_date date, is_topic_modified boolean, is_disabled boolean, is_deleteable boolean, created_at timestamp without time zone, is_inconsistent boolean, total bigint)
    LANGUAGE plpgsql
    AS $$
DECLARE
    offset_val INTEGER;
...
ALTER FUNCTION academic.fn_syllabus(p_perpage integer, p_npage integer, f_course_ids jsonb, f_university_ids jsonb, f_cycle_id bigint, f_cycle_active boolean, p_company_id bigint) OWNER TO postgres;
