-- Function: academic.fn_source_layers_on_subtopic(integer, integer, integer, integer)

--

CREATE FUNCTION academic.fn_source_layers_on_subtopic(p_perpage integer, p_n_page integer, p_university_id integer, p_subtopic_id integer) RETURNS TABLE(year character varying, option_name character varying, area_name text, total bigint)
    LANGUAGE plpgsql
    AS $$
                                DECLARE
                                    total_count INTEGER;
...
ALTER FUNCTION academic.fn_source_layers_on_subtopic(p_perpage integer, p_n_page integer, p_university_id integer, p_subtopic_id integer) OWNER TO postgres;
