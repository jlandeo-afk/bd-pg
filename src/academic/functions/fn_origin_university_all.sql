-- Function: academic.fn_origin_university_all(smallint, character varying)

--

CREATE FUNCTION academic.fn_origin_university_all(f_type smallint, p_name_text character varying) RETURNS TABLE(id smallint, code character varying, name character varying, slug character varying, type smallint, region_id smallint, region character varying, options jsonb, areas text, versions jsonb)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY
...
ALTER FUNCTION academic.fn_origin_university_all(f_type smallint, p_name_text character varying) OWNER TO postgres;
